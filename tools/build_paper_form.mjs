...
function roomSection(name, index) {
  const type = inferAreaType(name);
  const checks = defaultChecks(type, name);
  const rows = checks.map((check) => `
      <tr>
        <td class="code">${esc(check.code)}</td>
        <td class="check-name">${esc(check.name)}<div>${esc(check.category)}</div></td>
        <td class="box"></td>
        <td class="box"></td>
        <td class="box"></td>
        <td class="notes"></td>
  ...
  return `
    <section class="room">
      <div class="room-head">
        <div>
          <strong>${String(index + 1).padStart(2, "0")}. ${esc(name)}</strong>
          <span>${esc(areaTypeLabels[type])}</span>
        </div>
        <div class="room-flags">
          <span>נבדק □</span>
          <span>הוזן לאפליקציה □</span>
        </div>
      </div>
      <div class="dimensions">
        <span>רוחב בתכנית: __________</span>
        <span>אורך בתכנית: __________</span>
        <span>רוחב בפועל: __________</span>
        <span>אורך בפועל: __________</span>
      </div>
      <table>
        <thead>
          <tr>
      ...
      <div class="room-notes">הערות כלליות לחדר: ________________________________________________________________________________________________</div>
    </section>`;
}
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    header {
      ...
      align-items: center;
      padding-bottom: 8px;
      border-bottom: 2px solid #1d5f68;
      margin-bottom: 8px;
    }
    .logo { width: 138px; height: auto; }
    h1 { margin: 0 0 3px; font-size: 22px; line-height: 1.1; }
    .subtitle { margin: 0; color: #5f6f76; font-size: 12px; }
    .meta-grid {
      display: grid;
    ...
    .field strong { display: block; font-size: 9px; color: #466169; }
    .instructions {
      display: grid;
    ...
    .room-head strong { font-size: 13px; }
    .room-head span { color: #4d666e; }
    .room-flags {
      display: flex;
      gap: 12px;
      white-space: nowrap;
    ...
    th, td { border-bottom: 1px solid #e1e8ea; border-left: 1px solid #e1e8ea; padding: 3px 4px; vertical-align: top; }
    th { background: #f6f8f8; font-size: 9px; color: #35525b; }
    td { min-height: 22px; }
...
<body>
  ...
  <div class="meta-grid">
    <div class="field"><strong>דירה</strong></div>
  ...
  <div class="instructions">
    <div><strong>סימון:</strong> יש לסמן רק אחד: תקין / ליקוי / לא רלוונטי. אם יש ליקוי, חובה לכתוב הערה קצרה שאפשר להקליד לדוח.</div>
    <div><strong>תמונות:</strong> כתוב מספר תמונה או תיאור קצר, למשל 03, 04, 05. לאחר מכן העלה את התמונות לאותו סעיף באפליקציה.</div>
  </div>

  ${ownerAreaPreset.map(roomSection).join("\n")}

  <footer>לאחר החזרה לקליטה: לפתוח את הדירה באפליקציה, להקליד את הסימונים וההערות, להעלות תמונות, ואז לשמור.</footer>
</body>
</html>`;
